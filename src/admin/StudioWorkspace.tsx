...
type StudioView = "visual" | "dashboard" | "content" | "materials" | "cases" | "pages" | "settings" | "publish";
...
const nav: Array<{ id: StudioView; label: string; description: string; icon: typeof Home }> = [
  { id: "visual", label: "Edit website", description: "Drag and edit the page shown in the preview", icon: LayoutDashboard },
  { id: "content", label: "Revision topics", description: "Quizzes, vocabulary and detailed settings", icon: BookOpen },
...
export function StudioWorkspace(props: StudioWorkspaceProps) {
  const [view, setView] = useState<StudioView>("visual");
      ...
      <div className="studio-layout">
        ...
        <main className="studio-main">
          ...
          {view === "dashboard" && <section className="studio-dashboard">
            <div className="studio-page-heading"><span className="studio-kicker">Hello, {props.session.username}</span><h1>What would you like to improve?</h1><p>Choose one clear task. Every change is saved on this computer automatically, and nothing is published for students until you press Publish.</p></div>
            ...
            <div className="studio-quick-grid"><button type="button" onClick={() => setView("content")}><BookOpen /><span><strong>Edit a revision topic</strong><small>Choose a qualification, unit and revision topic. Add text, images, questions or sections.</small></span></button><button type="button" onClick={() => setView("materials")}><FolderKanban /><span><strong>Change materials</strong><small>Add or correct a specification, paper, video or revision link.</small></span></button><button type="button" onClick={() => setView("pages")}><FilePlus2 /><span><strong>Create a new page</strong><small>Build an extra page and choose whether it appears in the menu.</small></span></button><button type="button" onClick={() => setView("publish")}><CloudUpload /><span><strong>Check and publish</strong><small>Review warnings, connect to GitHub and publish the finished version.</small></span></button></div>
