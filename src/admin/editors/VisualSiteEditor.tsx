...
export function VisualSiteEditor({ bundle, onChange, assets, onAssetsChange }: VisualSiteEditorProps) {
  ...
  return <section className="visual-site-editor">
    ...
    <header className="visual-editor-heading">
      <div><span>Visual site editor</span><h1>Click a section to edit it</h1><p>Choose a page, click the section you want to change, then edit it in the panel. Drag the dotted handle to move it, or use the arrow buttons.</p></div>
    ...
    <ol className="visual-editor-guide" aria-label="How to use the visual editor"><li><span>1</span><strong>Choose a page</strong></li><li><span>2</span><strong>Click a section</strong></li><li><span>3</span><strong>Type or add an image</strong></li><li><span>4</span><strong>Check desktop and mobile</strong></li></ol>
...
function HomepageInspector({ block, updateBlock, addBlock, assets, uploadImage, settings, updateSettings, removeBlock }: {
  ...
  return <>
    ...
    <div className="visual-inspector-body">
      ...
      {block?.type === "image" && <div className="visual-inspector-fields"><Field label="Section heading"><input value={block.title} onChange={(event) => updateBlock((draft) => { if (draft.type === "image") draft.title = event.target.value; })} /></Field><Field label="Supporting text"><textarea rows={4} value={block.body ?? ""} onChange={(event) => updateBlock((draft) => { if (draft.type === "image") draft.body = event.target.value; })} /></Field><label className="visual-image-upload" onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); void uploadImage(event.dataTransfer.files[0]); }}><ImagePlus /><strong>{block.image.src ? "Drop or choose a replacement image" : "Drop an image here or choose a file"}</strong><span>PNG, JPEG or WebP · prepared automatically</span><input type="file" accept="image/png,image/jpeg,image/webp" onChange={(event) => void uploadImage(event.target.files?.[0])} /></label>{block.image.src && <img className="visual-inspector-image" src={imageSource(block.image.src, assets)} alt="Draft preview" />}<Field label="Alternative text"><textarea rows={3} value={block.image.alt} onChange={(event) => updateBlock((draft) => { if (draft.type === "image") draft.image.alt = event.target.value; })} /></Field><Field label="Caption"><input value={block.image.caption} onChange={(event) => updateBlock((draft) => { if (draft.type === "image") draft.image.caption = event.target.value; })} /></Field></div>}
