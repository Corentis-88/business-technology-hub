...
export function PublishPanel({ bundle, issues, pendingAssets, onPublish, onSaveToken, loadSavedToken }: Props) {
  ...
  const [message, setMessage] = useState("Update learning content through the Content Studio");
  ...
  const testAndSave = async (event: FormEvent) => {
    ...
    try {
      const response = await fetch("https://api.github.com/repos/Corentis-88/business-technology-hub", { headers: { Accept: "application/vnd.github+json", Authorization: `Bearer ${token.trim()}`, "X-GitHub-Api-Version": "2022-11-28" } });
  ...
  return <section className="studio-publish"><div className="studio-page-heading"><span className="studio-kicker">Final check</span><h1>Publish changes safely</h1><p>The live student site changes only after all checks pass and GitHub accepts one complete version.</p></div>
    ...
    <section className="studio-card studio-github-panel"><header><Github /><div><span>One-time publishing connection</span><h2>{checking ? "Checking this computer…" : tokenReady ? "GitHub is connected" : "Connect this computer to GitHub"}</h2></div>{tokenReady && <CheckCircle2 className="success-icon" />}</header>{checking ? <div className="studio-loading-inline"><LoaderCircle /> Checking securely…</div> : tokenReady ? <div className="studio-connected"><LockKeyhole /><p>The publishing key is encrypted on this computer. It can be opened only after the correct studio password is entered.</p><button type="button" onClick={() => { setTokenReady(false); setToken(""); setConnectionMessage(""); }}>Replace connection</button></div> : <form className="studio-token-setup" onSubmit={testAndSave}><ol><li><a href="https://github.com/settings/personal-access-tokens/new" target="_blank" rel="noreferrer">Open GitHub’s fine-grained token page <ExternalLink /></a></li><li>Name it <strong>Business and Technology Content Studio</strong> and choose this repository only.</li><li>Under Repository permissions, set <strong>Contents</strong> to <strong>Read and write</strong>.</li><li>Create the token, copy it, and paste it below. GitHub shows it only once.</li></ol><label><span><KeyRound /> GitHub token</span><input type="password" autoComplete="off" value={token} onChange={(event) => setToken(event.target.value)} placeholder="github_pat_…" required /></label><button className="studio-primary-button" type="submit" disabled={checking}>Check and save connection</button></form>}{connectionMessage && <div className="studio-message studio-message--notice"><CheckCircle2 />{connectionMessage}</div>}</section>
