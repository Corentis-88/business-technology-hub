...
export function ExtendedWritingPage() {
  ...
  return (
    <div className="extended-writing-page" style={{ "--course-accent": "#2f68b2" } as React.CSSProperties}>
      <header className="extended-writing-hero"><div className="page-section"><nav className="breadcrumbs" aria-label="Breadcrumb"><Link to="/">Home</Link><ChevronRight size={14} /><Link to="/course/business">Business GCSE</Link><ChevronRight size={14} /><span>Extended writing</span></nav><span className="eyebrow">Pearson Edexcel Business only</span><h1>Build connected case-study answers</h1><p>Move from a precise case hook to a logical chain, counterweight and supported judgement. BLT is the teaching scaffold used here; the assessment guidance below is checked against Pearson materials.</p><div className="extended-writing-hero__actions"><Link className="button button--primary" to="/business/blt">Open BLT builder <ArrowRight size={16} /></Link><a className="button button--secondary" href={officialDocumentHref("pearson-business-sample-assessment-materials.pdf")}>View official sample materials</a></div></div></header>
      <main className="page-section extended-writing-content">
        ...
        <section className="case-lab"><div className="section-heading"><div><span className="eyebrow">Original fictional practice</span><h2>Case-study strand lab</h2></div><p>These fictional cases imitate the information structure of Pearson source booklets without copying a past-paper case.</p></div>
          ...
          <article className="source-booklet">
            <header><span>Original fictional practice case · Not a Pearson past paper</span><h2>{practiceCase.title}</h2><p>{practiceCase.theme}</p></header>
